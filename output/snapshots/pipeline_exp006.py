"""
pipeline.py  --  Experiment #6: Stacking + PCA features added
"""
import sys
import numpy as np
...
def run():
    ...
    for mouse in mice:
        ...
        X_train_aug = np.column_stack([X_train, pca_train])
        X_test_aug = np.column_stack([X_test, pca_test])
        ...
        y_pred[test_mask] = model.predict(X_test_aug)
    ...
    print(f"N_FEATURES={X.shape[1]}+{n_comp}PCA")
